...
> มี BA + SD ครบแล้ว, อยากให้ Stitch generate UI ที่มี personality ระดับ Vercel/Linear โดยไม่ต้องออกแบบ design system เอง — ใช้ user-supplied DESIGN.md ที่ acquired ตาม `.agents/development-guide/ux-design-reference-acquisition.md` เป็น visual language guide เสริม
...
```
Session 1: PM + tech lead เปิด .agents/development-guide/ux-design-workflow.md
           → อ่านตาราง "Reference Site Selection Guide" ใน Mode D section
           → cross-ref กับ .agents/development-guide/ux-design-reference-acquisition.md §6
```
...
```
Read: .agents/prompt-templates/ux-design-stitch-prompt.md
...
This design vision is customized from user-supplied references at `design-reference/` (acquired per `.agents/development-guide/ux-design-reference-acquisition.md`):
...
|------|---------|----------|------|
| Acquire references | Browser save จาก getdesign.md (Option 2) → paste ที่ `design-reference/<name>-DESIGN.md` ตาม [acquisition guide](../.agents/development-guide/ux-design-reference-acquisition.md) | UX lead | ~10 min |
