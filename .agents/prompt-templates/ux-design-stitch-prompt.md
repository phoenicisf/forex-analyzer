...
| **StitchMCP** `edit_screens` | Edit existing screens with targeted adjustments |
| **StitchMCP** `list_projects` / `get_project` | Manage Stitch projects |
| **StitchMCP** `list_screens` / `get_screen` | Browse and retrieve screen assets |
...
| **Brainstorming Visual Companion** | Show wireframe options in browser for user selection |
...
1. ตรวจ `design-reference/<name>-DESIGN.md` ที่ user/operator วางไว้แล้ว
   - ถ้ายังไม่มี → operator ต้อง acquire ก่อนตาม `.agents/development-guide/ux-design-reference-acquisition.md` (4 options: npx getdesign / browser save / copy from another project / hand-author) — workflow ห้าม fetch อะไรเอง
