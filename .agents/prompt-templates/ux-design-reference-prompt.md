...
- Read user-supplied DESIGN.md files from `design-reference/<name>-DESIGN.md` (acquisition is operator-side per `.agents/development-guide/ux-design-reference-acquisition.md` — you do NOT fetch/download/install anything)
...
> **Acquisition is operator-side, not your job.** If `design-reference/` is empty or schema-incomplete → HALT and point user to `.agents/development-guide/ux-design-reference-acquisition.md`. ห้าม curl/fetch/`npx getdesign`/network call ใดๆ ภายใน workflow
...
> **Note:** Acquisition (วิธีได้ DESIGN.md มา) ไม่ใช่ส่วนหนึ่งของ workflow นี้ — operator ทำตาม `.agents/development-guide/ux-design-reference-acquisition.md` แล้ววางไฟล์ลงตาม convention ก่อนรัน workflow Workflow ตรวจ + บริโภคเท่านั้น
...
Phase 2: VERIFY & VALIDATE USER-SUPPLIED REFERENCES
  ...
  │     ├── If empty → HALT + point user to `.agents/development-guide/ux-design-reference-acquisition.md`
...
- **No acquisition inside the workflow** — ห้าม curl/fetch/`npx getdesign`/network call ใดๆ ภายใน prompt execution Acquisition คือ operator-side action ตาม `.agents/development-guide/ux-design-reference-acquisition.md`
