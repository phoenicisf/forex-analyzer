...
The MT5 install path is stored in `origin.txt` at the project root (one line, e.g. `C:\Program Files\FBS MetaTrader 5ph`). **Always read it dynamically** instead of hardcoding a broker name:
