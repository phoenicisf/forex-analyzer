...
> 🔧 **ก่อนเริ่ม:** ตรวจให้แน่ใจว่า Stitch MCP server ตั้งค่าถูกต้อง (proxy + API key, ไม่ใช่ direct HTTP). ถ้าเป็นครั้งแรก / ยังไม่เคย verify → ดู [`./ux-design-stitch-mcp-setup.md`](./ux-design-stitch-mcp-setup.md) ทำตาม §2 (clean install) หรือ §3 (recovery) ก่อนรัน workflow. ถ้า Stitch MCP ใช้ไม่ได้ทั้งหมด → fallback Mode D ตาม guide §6
