...
**📖 Guide:** `.andm/development-guide/ideate-workflow.md` (มีตัวอย่าง brief ฉบับเต็ม)
...
**ทำงานเบื้องหลัง:** workflow `/ba` อ่าน `.andm/prompt-templates/ba-requirements-prompt.md` (authoritative persona + 4-phase process + Language Rule + Readability Contract) + อ่าน `docs/foundation-input-sources/` อัตโนมัติ — ไม่ต้องเติม `[PLACEHOLDER]` ใน prompt
...
**ทำงานเบื้องหลัง:** workflow `/sd` อ่าน `.andm/prompt-templates/system-design-master-prompt.md` (authoritative persona + 6-step process + Phase Contract + Language Rule + Readability Contract) + อ่าน `docs/ba/01-05` (v1.2) + `docs/foundation-input-sources/` อัตโนมัติ — enforce schedule-leakage check ใน Phase 3 quality gate
