//+------------------------------------------------------------------+
//| slots/Slot_C.mqh โ€” Slot C implementation (IMPL-019)              |
//| Layer:   slots/ (inherits domain/CSlotBase; ADR-002 contract)     |
//| Magic:   MAGIC_CD = 200 (shared with Slot D โ€” D is 4-line wrapper)|
//| Source:  CodeWiki ยง3 Slot C + ยง4; ADR-002; ADR-008; ADR-012        |
//|                                                                   |
//| M-size MVP โ€” 5 of N CodeWiki ยง3.C entry conditions:               |
//|   1. No active C orders (magic 200 with "C," comment prefix)       |
//|   2. MACD M10: histogram direction (hist > 0 BUY / <0 SELL)        |
//|      CodeWiki ยง3.C โ€” MACD M10 histogram momentum signal            |
//|   3. ADX H4 dominance: adx > InpCAdxMin                            |
//|      CodeWiki ยง3.C โ€” ADX trend-strength filter                     |
//|   4. Stochastic H4 oversold/overbought confirmation                |
//|      CodeWiki ยง3.C โ€” Stochastic entry timing filter                |
//|   5. C-Pending integration: EnterPending/GetState/TransitionExec   |
//|      CodeWiki ยง3.C โ€” pending retest gate (ADR-008 PM_C machine)    |
...
//|                                                                   |
//| Exit (ManageExits):                                               |
//|   - Profit gate โฅ InpCTpProfitPips (40 pip default)                |
//|                                                                   |
...
//| Comment: "C,MA,N,1,SL" per CodeWiki ยง3.C comment format           |
//| Shared-magic disambig: GetTicketsForSlot(MAGIC_CD, "C,", tickets) |
//|   so D's "D," orders are excluded (BR-4.4 / ยง4.4)                 |
//|                                                                   |
...
//+------------------------------------------------------------------+
//| CSlotC โ€” Slot C derived class (ADR-002 CSlotBase contract)        |
//|                                                                   |
...
public:
   ...
   //--- 1. Magic โ€” MAGIC_CD = 200 (shared with Slot D)
   virtual int           Magic()  const override { return MAGIC_CD; }

   //--- 2. SlotId โ€” "C"; journal slot_id + comment prefix disambig from "D,"
   virtual string        SlotId() const override { return "C"; }

   //--- 3. Evaluate โ€” entry pass with C-Pending integration (FR-2.3)
   ...
   //--- 5. DependsOn โ€” C is CD chain root (topologically independent)
   ...
   //--- 6. PendingState โ€” delegate to PMR PM_C; safe NULL guard fallback
...
//+------------------------------------------------------------------+
//| _HasActiveCOrder โ€” check for open C orders via PortfolioState     |
...
//+------------------------------------------------------------------+
//| _IsCBuyBaseSignal โ€” base BUY signal for pending gate              |
//|                                                                   |
//| Conditions (MVP 3 of N CodeWiki ยง3.C):                            |
//|   1. MACD M10 histogram > threshold โ€” CodeWiki ยง3.C MACD momentum |
//|   2. ADX H4 dominance: adx > InpCAdxMin โ€” CodeWiki ยง3.C ADX trend |
//|   3. Stochastic H4 oversold: k < InpCStochOversold โ€” CodeWiki ยง3.C|
//+------------------------------------------------------------------+
bool CSlotC::_IsCBuyBaseSignal(const MarketContext &ctx) const
  {
   // CodeWiki ยง3.C โ€” MACD M10 histogram momentum signal (BUY)
   bool macd_buy = ctx.macd_m10.hist > InpCMacdSignalThresh;
   // CodeWiki ยง3.C โ€” ADX trend-strength filter
   bool adx_ok   = ctx.adx_h4.adx > InpCAdxMin;
   // CodeWiki ยง3.C โ€” Stochastic entry timing filter (oversold)
   bool stoch_ok = ctx.stoch_h4.k_main < InpCStochOversold;
   return macd_buy && adx_ok && stoch_ok;
  }

//+------------------------------------------------------------------+
//| _IsCSellBaseSignal โ€” base SELL signal for pending gate (mirror)   |
//+------------------------------------------------------------------+
bool CSlotC::_IsCSellBaseSignal(const MarketContext &ctx) const
  {
   // CodeWiki ยง3.C โ€” MACD M10 histogram momentum signal (SELL)
   bool macd_sell = ctx.macd_m10.hist < -InpCMacdSignalThresh;
   // CodeWiki ยง3.C โ€” ADX trend-strength filter
   bool adx_ok    = ctx.adx_h4.adx > InpCAdxMin;
   // CodeWiki ยง3.C โ€” Stochastic entry timing filter (overbought)
   bool stoch_ok  = ctx.stoch_h4.k_main > InpCStochOverbought;
   return macd_sell && adx_ok && stoch_ok;
  }

//+------------------------------------------------------------------+
//| _IsCBuyTrigger โ€” trigger condition to execute pending BUY         |
...
//+------------------------------------------------------------------+
//| _IsCSellTrigger โ€” trigger condition to execute pending SELL       |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Evaluate โ€” Slot C entry pass with C-Pending integration           |
//|                                                                   |
//| C-Pending pattern (ADR-008 / BR-6.1 / shared context ยง4.3):       |
//|   Phase A (base signal, not yet in pending):                      |
//|     IDLE + base signal โ’ EnterPending(PM_C, payload, bar_index)  |
//|   Phase B (pending, trigger now valid):                           |
//|     PENDING + trigger valid โ’ place entry + TransitionExecuted    |
//|   Force-clear: PMR.TickAll (Orchestrator step 8) โ€” slot เธซเนเธฒเธก poll |
//|   Legacy timeout: InpLegacyCBars (8 H4 bars) โ€” PMR handles via   |
...
void CSlotC::Evaluate(const MarketContext &ctx, CPortfolioState &port)
  {
   ...
   //--- Condition 1: no active C orders ("C," prefix โ€” shared MAGIC_CD with D)
   ...
   if(st == PENDING_STATE_PENDING)
     {
      ...
      //--- Pip size via base-class helper (Round-06 06.1 โ€” ea.md mandate)
      double pip_size = _PipSize();
      double sl_pips  = InpCSlPipsFloor;

      //--- Compute lot via RiskManager (เธซเนเธฒเธก instantiate CTrade direct per ea.md)
      ...
      if(lot <= 0.0)
        {
         m_logger.Warn("SlotC", "zero_lot_skip", MAGIC_CD,
                       "ComputeLot returned 0 โ€” skipping C entry");
         return;
        }
      ...
      req.comment      = comment;
      req.magic        = MAGIC_CD;
      req.type_filling = ORDER_FILLING_FOK;  // broker filling detection at Orchestrator wiring path (core/Orchestrator.mqh)
...
//+------------------------------------------------------------------+
//| ManageExits โ€” Slot C exit pass (CodeWiki ยง3.C / ยง4 MVP)           |
//|                                                                   |
//| Exit logic (MVP):                                                 |
...
//|   3. Profit gate โฅ InpCTpProfitPips (40 pip default) โ’ close     |
//+------------------------------------------------------------------+
void CSlotC::ManageExits(CPortfolioState &port)
  {
   if(m_logger == NULL) return;

   //--- Retrieve C tickets only (comment prefix "C," โ€” shared MAGIC_CD with D)
   ulong tickets[];
   int n = port.GetTicketsForSlot(MAGIC_CD, "C,", tickets);
   if(n <= 0) return;
   ...
   for(int i = 0; i < n; i++)
     {
      ...
      //--- Profit gate: โฅ InpCTpProfitPips โ’ emit close signal
