...
> **Action ถัดไป:** ~~`/impl-plan-review all`~~ ✅ **R06 closed 2026-05-03**; ~~IMPL-039 (Slot_BI G4 SL fix ADR-009)~~ ✅ **closed 2026-05-04**; ~~IMPL-034 (Slot_P A7 risk)~~ ✅ **closed 2026-05-04**; ~~IMPL-013 (21 per-slot input files rolling-close)~~ ✅ **closed 2026-05-04** (3/3 S-AC `[x]` via filesystem grep — 21 files / 21 group annotations / 178 input declarations; 1 E-AC `[x]` file-blob-check; 1 E-AC deferred to IMPL-060 entry .mq5 → 1 new Active row expiry 2026-05-18). **P3 Phase Tier 1 = 23/23 ✅** — Phase Gate now nominate-able. ~~`/impl-task IMPL-053`~~ ✅ **closed 2026-05-04** (M [ea] CrossSlotCoordinator::RunSafePort BR-8.1 — first P4 task under Phase Gate Override Path A; full RunSafePort body iterates 11 target entries `{C/D/J/H/K/L/M/Q/GO/T/S}` via CTrade.PositionClose + journal `triggering_function="OrderGroupStartWorkflow"`; sibling cross-slot stubs guarded for IMPL-054..057; Spike_CrossSlotCoordinator G1 0err/0warn/838 ms; 7/7 SelfTest cases pass; 4/4 S-AC `[x]`; 1 E-AC deferred → 1 new Active row expiry 2026-05-18). **P4 Phase Status snapshot 0/11 → 1/11**. **Next:** IMPL-055 (S RunForceCutloss BR-8.3 CD pair — simplest in chain) **OR** IMPL-054 (M RunOrderGroup2 BR-8.2 Ichimoku) **OR** IMPL-056 (XS ExtraCheckFunction2 BR-8.5) — same-file `services/CrossSlotCoordinator.mqh` scope so sequential; after IMPL-058 → IMPL-059 (L Orchestrator) + IMPL-060 (S entry .mq5) → empirical surface unblocked for P2/P3/P4 Phase Gates. Code Review trigger R09: after IMPL-058 chain complete (~5 P4 tasks) for adversarial sweep on cross-slot surface + ADR-010 HALTED enable matrix verification. P2 + P3 Gates retroactively close once IMPL-059+ Orchestrator skeleton lands + Tier 1.5 walk via `p2_services_smoke.ini` + 60-day slot smoke produces evidence artifacts.
> **Deferred-AC Active:** **6 P1 rows (backfilled 2026-05-03 post-R06 Claim 06.2)** + 5 P2 rows + 24 P3 E-AC deferrals (IMPL-013 ×1 + IMPL-019/020/021/022 ×2/023/024/025/026/027/028/029/030/031/032/033/034/035/036/037/038/039 ×2 — smoke 60-day backtest + IMPL-022/IMPL-039 G4 attestation journal evidence + IMPL-013 input dialog probe) + **1 P4 row (IMPL-053 smoke 10-position fixture E-AC; expiry 2026-05-18; block on IMPL-059+ Orchestrator + entry .mq5)** = **36 Active rows total** · expiry 2026-05-17 (P1/P2/most P3) and 2026-05-18 (IMPL-013 + IMPL-034 + IMPL-039 + IMPL-053 rows) · all blocked on IMPL-059+ Orchestrator + entry .mq5 (IMPL-060)
> **Last updated:** 2026-05-04 · last action: **IMPL-059 closed (L [ea] core/Orchestrator composition root + OnInit 3-phase + OnTick F1 14-step + CleanupPartialInit reverse-order release)** — single-task `/impl-task IMPL-059` Phase 2C 12-step decomposition. NEW: `core/Orchestrator.mqh` (740+ LOC verbatim TD-02 §7.1-7.4.1) + `spike/Spike_Orchestrator.mq5` + `simulation/headless-tests/orchestrator_smoke.ini`. EDIT: `core/SlotRegistry.mqh::RegisterAll` body (stub → 21-slot heap-new in BR-2.2 topo + 21 slot includes); `services/IndicatorService.mqh` + `services/CircuitBreaker.mqh` ODR fix (24 + 2 `static const int` decl/def split); `(void)scan_fn` cast → `scan_fn=scan_fn` idiom. G1 0err/0warn/608 ms (Spike_Orchestrator); 26/26 sibling spike regression sweep all 0err/0warn (5 service spikes + 21 slot spikes). 7/7 S-AC `[x]`; 3 E-AC deferred (combined Phase C deliberate-fail + step-ordering + step-5b precedence smoke — needs IMPL-060 entry .mq5 + Tester run) → 1 new Active row in `deferred-ac-registry.md` expiry 2026-05-18. 5 spec deviations from TD-02 §7.4 (D-1..D-5) all driven by service-actual signature divergence; documented in Orchestrator.mqh header banner. P4 6/11 → **7/11**. Mid-Phase Audit P4 counter = 2. **Plan Staleness Sentinel = 10 closures since R06 — TRIPS THRESHOLD** → strongly recommend `/impl-plan-review all` before IMPL-060. Evidence: `_session-handoff/IMPL-059-evidence-20260504.md`. **Next:** `/impl-review all` + `/impl-plan-review all` THEN `/impl-task IMPL-060` (S entry .mq5 thin wrapper — closes runnable-surface gap, unblocks 36+ deferred-AC rows expiring 2026-05-17/18). Prior action: **IMPL-053 closed (M [ea] — `services/CrossSlotCoordinator::RunSafePort` BR-8.1 OrderGroupStartWorkflow)** (single-task `/impl-task IMPL-053` Phase 2B 3-step; first P4 task closed under Phase Gate Override 2026-05-03 Path A. NEW × 3: `services/CrossSlotCoordinator.mqh` (full skeleton 7 methods + RunSafePort body + sibling stubs guarded for IMPL-054..057), `spike/Spike_CrossSlotCoordinator.mq5` (7 SelfTest cases pass), `simulation/headless-tests/cross_slot_safe_port.ini` (smoke fixture per TD-02 §13.6). RunSafePort body: `_AggregateWeakMetrics` iterates `PositionsTotal()` filtered `_Symbol==EURUSD` (NFR-5.3) → derives weak_count/sum_bad_pip/total_pl; `_SafePortTriggered` AND of (weak_count>1, avg_bad_pip>55, combined_pl>0) per BR-8.1 spec literal; when triggered, `_FillSafePortTargets` populates 11 entries `{(MAGIC_CD,"C,"), (MAGIC_CD,"D,"), (MAGIC_J,"J,"), (MAGIC_H,"H,"), (MAGIC_K,"K,"), (MAGIC_L,"L,"), (MAGIC_M,"M,"), (MAGIC_Q,"Q,"), (MAGIC_GO,"GO,"), (MAGIC_T,"T,"), (MAGIC_S,"S,")}` per BA `04 § BR-8.1`; per pair `_CloseSlotGroup` calls `port.GetTicketsForSlot` + `m_trade.PositionClose` + per-ticket journal `event_type="exit"` `triggering_function="OrderGroupStartWorkflow"`; returns int slots_closed_count. **Spec deviation logged**: TD-02 §5.11 declares `void`; impl returns `int` per S-AC #3 — Plan text > skeleton text per Plan QA precedent (header banner + audit log row). HALTED matrix per `04 § 9.1`: SafePort runs in BOTH RUNNING+HALTED; EOverload/TriggerGOverload guarded `if(m_halted) return;`. G1 ✅ Spike_CrossSlotCoordinator 0err/0warn/838 ms (PowerShell Start-Process MetaEditor64). SelfTest 7/7: C1 Init defaults, C2 SetHalted toggle, C3 zero-input gate false, C4 weak=2/avg=60/pl=10 true, C5 avg=40 false, C6 pl=-5 false, C7 target table 11 entries (CD x2 + 9 unique). 4/4 S-AC `[x]`; 1 E-AC deferred (smoke 10-position fixture — block on IMPL-059+ Orchestrator + entry .mq5 + portfolio populator OnTradeTransaction wiring) → 1 new Active row in `deferred-ac-registry.md` expiry 2026-05-18. **P4 Phase Status snapshot 0/11 → 1/11**. Mid-Phase Audit P4 counter = 1. **Plan Staleness Sentinel = 4 closures since R06** (well below 10-closure threshold). Newly unblocked: IMPL-054/055/056/057/058 (same-file scope — sequential). Evidence: `_session-handoff/IMPL-053-evidence-20260504.md`. **Next:** IMPL-055 (S RunForceCutloss BR-8.3 CD pair — simplest) **OR** IMPL-054 (M RunOrderGroup2 BR-8.2 Ichimoku) **OR** IMPL-056 (XS ExtraCheckFunction2 BR-8.5). Prior: **IMPL-013 closed (L [ea] — 21 `inputs/Inputs_Slot_<X>.mqh` files rolling-close)** (single-task `/impl-task IMPL-013`; formal AC marking after IMPL-034 Slot_P shipped final `Inputs_Slot_P.mqh` 2026-05-04 → 21/21 file set complete. No new code shipped — file set rolled in incrementally per IMPL-013 description engineer convention bundled with IMPL-019..039 commits. 3/3 S-AC `[x]` via filesystem grep: (1) 21 files exist + each has `input group "Slot <X>"` annotation per NFR-6.3; (2) defaults match CodeWiki §3 baseline rolling via 21/21 Spike_Slot_X G1 0err/0warn; (3) **178 per-slot input declarations** + 22 IMPL-012 General + ≥ 15 IMPL-014 cross-slot = ≈ 215 cumulative ≫ NFR-4.3 ≥ 80 target. 1 E-AC `[x]` file-blob-check (178 grep count); 1 E-AC deferred to IMPL-060 entry .mq5 (live MT5 input dialog probe needs chart attach) → 1 new Active row in `deferred-ac-registry.md` expiry 2026-05-18. Evidence: `_session-handoff/IMPL-013-evidence-20260504.md`. P3 Phase Status snapshot **22/23 → 23/23 ✅** — P3 Phase Gate becomes nominate-able pending IMPL-053+ Orchestrator chain unblocking Tier 1.5 walk + empirical demo. Mid-Phase Audit P3 counter = 23 (advisory pending runnable surface). **Plan Staleness Sentinel = 3 closures since R06** (IMPL-039 + IMPL-034 + IMPL-013 — well below 10-closure threshold). Prior: **IMPL-034 closed (L [ea] Slot_P — ⚠️ A7 risk sub-modes PSUB_NONE/N/PX/PH/E per `04 § 4.4`; legacy timeout BR-6.4)** (single-task `/impl-task IMPL-039`; Phase 2C 7-step decomposition: inputs → Slot_BI skeleton + 6 overrides → Evaluate (G4 fix surface) → ManageExits → spike + smoke ini → G1 → state propagation. CSlotBI inherits CSlotBase; Magic=MAGIC_B=214 (shared with B parent); SlotId="BI"; comment "BI,pyr,1"; DependsOn=0 (runtime-state via PortfolioState; not topology — Slot_LX precedent); PendingState=IDLE. **G4 fix per ADR-009 Option A:** earliest-opened B parent (`parent_tickets[0]`) = base risk anchor; `bi_entry ± _PipsToPrice(parent_pip_distance)` via CSlotBase helpers (Round-06 06.1); fallback `InpBISlFallbackPips=80` pip floor when `parent_sl=0` or no parent active (Bollinger fallback BBBot-10/BBTop+10 deferred to IMPL-062 P4 — needs M15 BB in MarketContext). **Spec deviation logged:** S-AC #3 plan text anchored SL at parent's open price; ADR-009 Option A locks anchor at BI's entry — implementation follows ADR-009 (architectural primary); deviation documented in `g4-fix-attestation.md § Fix #2` + Slot_BI.mqh header banner. Pyramid logic mirrors Slot_LX: own-no-active "BI," guard + parent-B-active "B," gate + profit_pips ≥ InpBIPyramidGatePips=30 + direction inheritance. ManageExits 30-pip profit gate over "BI,"-tagged tickets only. `Inputs_Slot_BI.mqh` 6 inputs. Spike_Slot_BI 6 SelfTest cases pass (G1 0err/0warn/425 ms). Sibling regression Spike_Slot_B/BR/LX/J 4/4 0err/0warn unchanged. **g4-fix-attestation.md NEW** — consolidated audit trail: Fix #1 IMPL-022 BR-7.2 (commit d386ea6) + Fix #2 IMPL-039 ADR-009 (commit pending this commit hash); IMPL-022 deferred-AC row "g4-fix-attestation.md does not yet exist" partially resolved (file exists; live journal evidence still pending IMPL-053+). 7/7 S-AC `[x]`; 2 E-AC deferred (smoke `[db-inspect]` + g4 attestation journal evidence path) → 2 new Active rows in `deferred-ac-registry.md` expiry 2026-05-18. P3 21/23 (Slot_P + IMPL-013 input completion remaining). Mid-Phase Audit P3 counter = 21. **Plan Staleness Sentinel = 1 closure since R06 review** (R06 closed 2026-05-03 reset to 0; +IMPL-039 = 1)). Prior: **Code Review Round 06 + Fix Round 06 applied** (4 findings — 3 full Accept + 1 Partial: 06.1 HIGH wired CPipMath into CSlotBase via SetPipMath setter + 4 protected helpers; 19 slots refactored to use inherited helpers — drift surface 19→1; 06.2 MEDIUM Slot_H `InpHEnabled`→`InpEnableSlotH`; 06.3 MEDIUM Partial — sl_price normalized in B/K/L/H, LX/S Reject with file-content evidence; 06.4 LOW SelfTest Case 5 split into 5a NULL-guard + 5b capacity overflow. Latent fix: PipMath `(void)x;` MQL5-illegal cast → `x=x;` idiom. G1 20/20 affected spikes 0err/0warn. See `docs/code-review/fix-round-06.md`. Plan Staleness Sentinel reset to 0 closures since last review.). Prior: **`rebuttal-round-06.md` closed** (claim-review-06: 7/7 Accept; 2 CRITICAL + 3 HIGH + 2 MEDIUM resolved — forbidden-pattern grep 0 hits restored; 6 P1 deferred-AC rows backfilled to registry; placeholder evidence paths converted to `<TBD-...>` markers; commit hashes substituted on IMPL-018/052; R-6 P2 retroactive-close timeline risk added). Prior: **Code Review Round 05 + Fix Round 05 applied** (10/10 findings accepted; Slot_H/B/K/L/BR/J + core/SlotRegistry refactored — Slot_H stripped CTrade member + naked Buy/Sell→log-intent stubs with computed sl_price (05.1/05.4/05.9), Slot_B/K/L exit loops switched Order*→Position* APIs (05.2/05.8), Slot_BR _CountBROrders honours InpBRMaxOrders (05.3), Slot_J +InpEnableSlotJ gate +remove dead j_state read (05.5/05.6), CSlotRegistry::Init routed via ReleaseAll (05.10), IMPL-023/024/025 added to Deferred-AC Active table (05.7); G1 7/7 affected spikes 0err/0warn + 13/13 sibling regression clean; 6 fix commits `01f3396`/`b102a0c`/`8a44ca2`/`7e62dbe`/`3266fd7`/`dca5e98`. **Plan Staleness Sentinel @ 48 closures since last review — STRONGLY recommend `/impl-plan-review all` BEFORE IMPL-039 BI SL fix (the second G4 fix per ADR-009 — Bucket B attestation point).**
...
- **R-6 P2 Phase Gate retroactive-close timeline risk** — Phase Gate Override Log row (2026-05-03) defers P2 close until IMPL-018+ ✅ + IMPL-053..060 chain (6 P4 tasks) lands + Tier 1.5 walk + drain 5 P2 deferred-AC rows. Timeline arithmetic: 5 P2 rows expire **2026-05-17** (first deadline) + single renewal allows extension to **2026-05-31** (final hard-stop). 6 P4 tasks (M+M+S+XS+XS+M) ≈ 28 days nominal + P3 still has 3 open tasks (IMPL-013/034/039 — IMPL-039 is HIGH RISK G4 fix). After R06 backfill, 6 additional P1 deferred-AC rows also expire 2026-05-17 (same renewal cycle). **Earliest mitigation:** prioritize IMPL-053 / IMPL-059 / IMPL-060 first in P4 batch order; consider re-scoping IMPL-053 RunSafePort fully fixturable (without RunOrderGroup2/RunForceCutloss) เพื่อ unblock walk earlier. **Hard-stop escalation:** ถ้า P2/P1 rows hit final renewal expiry 2026-05-31 without retroactive close → escalate via `/backtrack sd` to revisit P2 boundary.
...
- **E-AC**:
  - [ ] Smoke: open B parent + trigger BI pyramid → BI ticket has non-zero SL matching parent's pip distance `[db-inspect]` + `[log-assertion]` — **deferred to IMPL-053+ Orchestrator + RiskManager OrderSend wiring** per IMPL-018+ precedent (header-only `.mqh` until entry .mq5 + 60-day Tester run with B+BI active); registered in `deferred-ac-registry.md` Active row expiry 2026-05-17
...
- **Risk**: low
- **Rules**: `.claude/rules/ea.md`
- **Closed**: 2026-05-04 — `services/CrossSlotCoordinator.mqh` (RunForceCutloss body + `_ForceCutlossSignal` + `_CloseCDPositionsInLoss` helpers + SelfTest 7→13 cases) + `simulation/headless-tests/cross_slot_force_cutloss.ini` (NEW). Evidence: `_session-handoff/IMPL-055-evidence-20260504.md`. P4 Phase Status snapshot 1/11 → 2/11.
...
- **ADR**: ADR-010
- **Rules**: `.claude/rules/ea.md`
- **Closed**: 2026-05-04 — `services/CrossSlotCoordinator.mqh` (matrix doc block in header banner + SelfTest C26-C28 added; `[ev=overload_skipped_halted]` guards verified per ADR-010 :106) + `spike/Spike_CrossSlotCoordinator.mq5` (header banner refresh 25→28 cases). Evidence: `_session-handoff/IMPL-058-evidence-20260504.md`. P4 Phase Status snapshot 4/11 → 5/11.
...
**Last review on:** 2026-05-03 — `claim-review-06.md` + `rebuttal-round-06.md` (7/7 Accept; 0 Partial / 0 Reject / 0 Escalate; 2 CRITICAL closure-discipline + state-reconciliation defects resolved + 3 HIGH propagation gaps + 2 MEDIUM placeholder/commit-hash hygiene fixed)
**Closures since last review:** 2 (R06 closed 2026-05-03; +IMPL-039 + IMPL-034)

> Per `/next` Check 5.8: plan staleness recommendation triggers when (approved > 30d ago) AND (no review OR > 10 closures since review). Currently: 2 closures since R06 (2026-05-03) = **within threshold** ✅. **Defender 10-marker sweep convention** (extended from 7 → 10 markers in R06 per Claims 06.4 / 06.6 / 06.7): markers 1-7 from R03 + #8 TL;DR Active count vs registry actual + #9 Phase Gate evidence/walk placeholder paths + #10 `Closed:` commit-hash placeholder substitution check. Pending P3: IMPL-013 formal rolling-close (21/21 input files shipped — mark per-slot input AC `[x]`) + IMPL-018+/053+ runnable surface for empirical drain.
