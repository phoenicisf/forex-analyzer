...
>
> **Next:** ~~IMPL-FIX-001 + IMPL-FIX-002~~ ✅ **closed 2026-05-04 (parallel batch)**. **THEN** Tier 1.5 walk batch-2 (operator close foreground MT5 + run `bootstrap_smoke.ini` ~10 min) — drains 13+ resolvable deferred-AC rows (P1/P2 init_ok + Pending machine round-trip + slot Evaluate paths + 2 new IMPL-FIX-001/002 G3 rows that need rerun to confirm zero-ERROR + non-zero S lot + log-volume drop) by citing existing `tier-1.5-walk-2026-05-04/abridged-tester-log.txt` + new rerun artifact. **THEN** `/impl-review all` R09 (cross-slot + Orchestrator + entry .mq5 + 2 FIX commits + walk findings = significant attack surface). **THEN** P4 QA chain IMPL-061..068.
...
- ☐ Tier 1.5 walk batch-2 — drain 13+ rows by citing existing artifact (no re-run needed for prior 11+) + run `bootstrap_smoke.ini` again post-FIX merge to confirm S entries fire (`lot > 0.01`) + zero `[ev=s_pct_tp_invalid]` + log file ≤ 100 MB
- ☐ Code Review Round R09 (`/impl-review all`) — recommended (cross-slot + Orchestrator + entry .mq5 + ODR fixes + Tier 1.5 walk findings = significant attack surface)
- ☐ P2 + P3 Phase Gate retroactive close — blocked on Tier 1.5 walk artifact ≤14d + drain 29 P2/P3 deferred-AC rows
- ☐ P4 tail: IMPL-017 + IMPL-061..068 (QA chain) — runs after Tier 1.5 walk drains registry
- ☐ P4 Phase Gate close — blocked on IMPL-061..068 chain complete
