...
- **R-1 IMPL-046 Atomic-write spike risk gate** — ADR-007 design (Option A vs B) ผูกกับ MT5 sandbox `FileMove` atomic guarantee; spike fail → 1-2 day rework + activate Option B 3-file double-buffered swap (TD-02 §4.4). **Earliest mitigation:** P1 first parallelizable task; run before IMPL-047/048/049 commit
...
- **S-AC**:
  - [ ] All 5 enums declared with stable values (persist ใน state.json + journal)
  - [ ] 17 magic constants declared per `domain/EnumTypes.mqh § magic numbers`
  - [ ] `MAGIC_U` ไม่ปรากฏ (deleted per OQ-8)
  - [ ] Include guard `#ifndef PHOENICISNEX_DOMAIN_ENUMTYPES_MQH` ครอบทั้งไฟล์
- **E-AC**:
  - [ ] `grep -c "^enum E" domain/EnumTypes.mqh` returns 5 `[log-assertion]`
  - [ ] `grep -c "^static const int MAGIC_" domain/EnumTypes.mqh` returns 17 (BR-1.1 invariant) `[file-blob-check]`
- **Deps**: IMPL-001
- **Risk**: low
...
- **S-AC**:
  - [ ] `Init()` reads `_Digits` once + caches multiplier
  - [ ] `ToPoints(20.0)` returns 200 on 5-digit broker (FBS Standard EURUSD = 5-digit)
  - [ ] No `==` double comparison anywhere (use `NormalizeDouble`)
- **E-AC**:
  - [ ] OnInit Logger Info "pip_math digit_multiplier=10" on FBS-Real `[log-assertion]`
- **Deps**: IMPL-001
- **Risk**: low
...
- **S-AC**:
  - [ ] 3 files exist under `inputs/`
  - [ ] Each declares ≥ 5 inputs with proper `group=` annotation
  - [ ] Total declarations contribute to ≥ 80 input target (NFR-4.3)
- **E-AC**:
  - [ ] MT5 attach EA → input dialog has 3 distinct groups (TimeGates / Pending / Logging) each w/ ≥ 5 entries `[probe]`
- **Deps**: IMPL-001
- **Risk**: low
- **Rules**: `.claude/rules/ea.md`
...
- **S-AC**:
  - [ ] Spike EA `simulation/headless-tests/atomic_write_kill.ini` committed
  - [ ] 1000 normal writes succeed (state.json parses cleanly each)
  - [ ] 100 simulated kills produce 0 half-written files (file always parses OR doesn't exist + temp file lingers)
  - [ ] ADR-007 amended with `## Spike Result` section + go/no-go decision + date
- **E-AC**:
  - [ ] `cat post_kill_run.txt | grep -c "state.json parse fail"` returns 0 across 100 kill trials `[boot-cold]` + `[file-blob-check]`
  - [ ] If Option B activated → `docs/adr/007-state-persistence-atomic-temp-rename.md § Option B activation` populated + IMPL-010/047/048 task notes appended (engineer follow-up)
- **Deps**: IMPL-001
- **Risk**: **high** (risk gate; cascades to IMPL-010/047/048/049)
