...
>
> **ตอนนี้:** P1 เริ่มแล้ว · 14/68 tasks ปิด · Phase Gate 0/9 rows ติ๊ก ทุก phase
...
> **Action ถัดไป:** `/impl-task IMPL-010` (S `[ea]` — AtomicFile helper, now unblocked by IMPL-046 Option A lock) ขนานกับ `/impl-task IMPL-016` (XS — BootstrapValidator stubs) และ `/impl-task IMPL-006` (M — MarketContextBuilder); จากนั้น IMPL-047/048/049 StatePersistence chain เปิดทาง
> **Deferred-AC Active:** 0 rows · earliest expiry: n/a
> **Last updated:** 2026-05-02 · last action: IMPL-046 atomic-write spike closed (E1 risk gate ✅ `OPTION_A_LOCKED` — 1000/1000 phase-1 atomic writes intact, 100/100 phase-2 simulated mid-write crashes left state.json untouched; ADR-007 §Spike Result amended; IMPL-010/047/048/049 chain unblocked); P1 progress 14/17 [x]; next: IMPL-010 (AtomicFile) parallel-eligible with IMPL-016 + IMPL-006
...
|-------|----------------|------------------|---------------|-------|
| P1 Foundation + High-Risk Spike | 🔄 14/17 [x] | ⏸ pending | ⏸ 0/9 rows | IMPL-001/002/003/004/005/007/008/009/011/012/014/015/042/**046** closed 2026-05-02 (4 parallel batches + IMPL-046 serial spike: Option A locked); next: IMPL-010 (AtomicFile) parallel-eligible with IMPL-016 + IMPL-006 |
...
- ☑ **Continue Track A — `/impl-task IMPL-010` (AtomicFile helper, now unblocked by IMPL-046 Option A lock)** parallel-eligible with `/impl-task IMPL-016` (BootstrapValidator stubs) and `/impl-task IMPL-006` (MarketContextBuilder). IMPL-046 closed 2026-05-02 with verdict `OPTION_A_LOCKED` (1000/1000 atomic writes + 100/100 simulated mid-write crashes both clean); ADR-007 §Spike Result amended; downstream IMPL-047/048/049 StatePersistence chain unblocked for Option A 1:1 implementation (no schema fork). P1 progress 14/17 — only IMPL-006/010/016 remaining (then 17/17 enables P1 Phase Gate close).
...
- **S-AC**:
  - [ ] `Build()` returns `MarketContext` by value (copy semantics; ADR-004 ~720 bytes acceptable)
  - [ ] All 24 struct fields populated (none left default-zero)
  - [ ] `derived` block computed from raw fields (no DRY violation)
- **E-AC**:
  - [ ] Run smoke test → log emit one tick's `MarketContext` shape via Logger Debug → assert all 24 fields non-default `[log-assertion]`
- **Deps**: IMPL-005
- **Risk**: low
- **ADR**: ADR-004
- **Rules**: `.claude/rules/ea.md`
...
- **S-AC**:
  - [ ] `WriteAtomic(string path, string content)` returns bool (false ถ้า any step fail)
  - [ ] `CleanupOrphanTmp(string path, CLogger*)` deletes orphan `<path>.tmp` files from prior failed write
  - [ ] No exception path; degrade-but-continue on failure (return false + Logger Error)
- **E-AC**:
  - [ ] Smoke: write `state.json` 100 times → `cat state.json | jq .` parses cleanly each iteration `[file-blob-check]`
  - [ ] Manual interrupt (SIGTERM mid-write) ผ่าน Strategy Tester run + check no half-written `state.json` after restart `[boot-cold]`
...
- **S-AC**:
  - [ ] Returns false ถ้า `_Symbol != "EURUSD"`
  - [ ] Logger.Error + Alert ครบทั้งคู่ก่อน return false
- **E-AC**:
  - [ ] Attach EA on `GBPUSD` chart (test) → OnInit returns INIT_FAILED + Alert popup + journal `[ev=halt][reason=symbol_not_whitelist]` `[probe]` + `[log-assertion]`
...
- **S-AC**:
  - [ ] All 21 slot formulas dispatched (no `default: return 0;` silent fallback)
  - [ ] J/BI/I formulas read from `m_portfolio.GetByMagic(...)` per Claim 02.1
  - [ ] Returns NormalizeDouble lot sized to broker SYMBOL_VOLUME_STEP
- **E-AC**:
  - [ ] Smoke: invoke `ComputeLot("J", sl_pip=20, balance=10000, multiplier=1.0)` against fixture portfolio state → result matches CodeWiki §4.1 row J expected within ±0.01 lot `[log-assertion]`
  - [ ] Logger Debug emits per-slot lot calc result for first 21 slot evaluations `[log-assertion]`
...
- **S-AC**:
  - [ ] Returns clamped lot ≥ SYMBOL_VOLUME_MIN AND ≤ cap
  - [ ] Logger Warn ถ้า raw outside [floor, cap]
- **E-AC**:
  - [ ] Smoke: invoke ClampLot(raw=99.0, slot=C) on $1000 balance → returns ≤ cap calculated from BR-4.2 `[log-assertion]`
- **Deps**: IMPL-040
- **Risk**: low
- **Rules**: `.claude/rules/ea.md`
...
- **S-AC**:
  - [ ] `Update` increments worst_dd_pct ถ้า current DD < stored
  - [ ] Persists `worst_dd_pct` ใน `state.json § watch_profits` per `state-persistence-schema.yaml`
  - [ ] No halt trigger (OQ-6 monitor-only)
- **E-AC**:
  - [ ] Smoke: simulate equity drop $10000→$9000 → `state.json § watch_profits.worst_dd_pct` updates to 10.0 `[db-inspect]`
...
- **S-AC**:
  - [ ] 35 fields across 11 sub-objects documented
  - [ ] `schema_version: 1` lock
  - [ ] If Option B activated → 3-file schema variant added under `## Option B Layout`
- **E-AC**:
  - [ ] `yq eval '... | length' state-persistence-schema.yaml` matches expected counts `[file-blob-check]`
...
- **S-AC**:
  - [ ] 5 methods + cycle through ban cooldowns per slot
  - [ ] DST-aware: `Mar 28, 2021 00:00 GMT+2 → 03:00 GMT+3` transition handled correctly
- **E-AC**:
  - [ ] Smoke: backtest 2026-Mar-28 (DST start) + 2026-Oct-25 (DST end) → no `IsMorningWakeup` / holiday off-by-1-hour bugs `[log-assertion]`
  - [ ] state.json `bans` sub-object updates after slot ban triggered `[db-inspect]`
...
- **S-AC**:
  - [ ] Returns true on detected ping-pong; tracks last 5 close events with timestamps
  - [ ] Logger.Warn emitted on any near-miss (< 5000 ms) for visibility
- **E-AC**:
  - [ ] Smoke: stub portfolio with 3 close events 1500ms apart → `CheckPingPong` returns true → EAState transitions HALTED `[log-assertion]`
- **Deps**: IMPL-042
...
**Closures since last review:** 14 (IMPL-001 + parallel batch #1 [002+009+014] + parallel batch #2 [003+004+008] + parallel batch #3 [011+012+042] + parallel batch #4 [005+007+015] + IMPL-046 atomic-write spike serial, all closed 2026-05-02)

> Per `/next` Check 5.8: plan staleness recommendation triggers when (approved > 30d ago) AND (no review OR > 10 closures since review). Currently: 14 closures = **threshold exceeded** (still ≤30d window). **Recommend running `/impl-plan-review all` + `/impl-review all` now** that IMPL-046 (E1 risk gate) has closed and unblocked the AtomicFile/StatePersistence chain — re-validate plan against the Option A lock outcome before P1 Phase Gate close (only IMPL-006/010/016 remaining); not blocking IMPL-010/006/016 if user prefers to defer review until P1 completion.
