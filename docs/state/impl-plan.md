...
>
> **Next:** **IMPL-FIX-001 hot-fix** (HIGH — Slot_S percent_tp threading; Tier 1.5 walk 2026-05-04 surfaced; 3-line fix in `Inputs_Slot_S.mqh` + `Slot_S.mqh:203`). **THEN** IMPL-FIX-002 (MEDIUM — `clamp_applied` log hygiene). **THEN** Tier 1.5 walk batch-2 to drain remaining 11+ resolvable deferred-AC rows (P1/P2 init_ok + Pending machine round-trip + slot Evaluate paths) by citing `tier-1.5-walk-2026-05-04/abridged-tester-log.txt`. **THEN** `/impl-review all` R09. **THEN** P4 QA chain IMPL-061..068.
...
- ☐ **`/impl-task IMPL-FIX-001`** (S [ea] HOT-FIX — Slot_S percent_tp threading; needed before next walk batch to clear log noise)
- ☐ **`/impl-task IMPL-FIX-002`** (S [ea] log hygiene — `clamp_applied` rate-limit or DEBUG demote)
- ☐ Tier 1.5 walk batch-2 — drain remaining 11+ rows by citing existing artifact (no re-run needed) + run `bootstrap_smoke.ini` again post IMPL-FIX-001 to confirm S entries fire
...
- **S-AC**:
  - [ ] `Inputs_Slot_S.mqh` declares `InpSPercentTp = 10.0` with `group "Slot S"` annotation
  - [ ] `Slot_S.mqh:203` passes `InpSPercentTp` as 4th positional arg to `m_risk.ComputeLot`
  - [ ] G1 compile 0err/0warn
- **E-AC**:
  - [ ] G3 headless run via `bootstrap_smoke.ini` shows zero `[ev=s_pct_tp_invalid]` ERROR lines `[log-assertion]`
  - [ ] At least one `[ev=entry_signal][slot=SlotS]` with `lot > 0.01` (not floor-clamped) in same run `[log-assertion]`
- **Deps**: IMPL-060 (runnable surface needed for E-AC verification)
...
- **S-AC**:
  - [ ] One of (a)/(b)/(d) chosen + documented in commit Why; (c) requires `/impl-plan-review` escalation
  - [ ] G1 compile 0err/0warn
- **E-AC**:
  - [ ] G3 headless run via `bootstrap_smoke.ini` shows ≤ 1 `[ev=clamp_applied]` per slot per session OR log file size ≤ 100 MB for the same 3-day window `[log-assertion]` + `[file-blob-check]`
