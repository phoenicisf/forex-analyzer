...
> **Deferred-AC Active:** **6 P1 rows (backfilled 2026-05-03 post-R06 Claim 06.2)** + 5 P2 rows + 24 P3 E-AC deferrals (IMPL-013 ×1 + IMPL-019/020/021/022 ×2/023/024/025/026/027/028/029/030/031/032/033/034/035/036/037/038/039 ×2 — smoke 60-day backtest + IMPL-022/IMPL-039 G4 attestation journal evidence + IMPL-013 input dialog probe) + **9 P4 rows (IMPL-053 + IMPL-054 + IMPL-055 + IMPL-056 + IMPL-057 + IMPL-058 + IMPL-059 + IMPL-060 individual smoke/matrix/composition/init_ok E-ACs + IMPL-053..056 compound close-path empirical row from fix-round-09 Finding 09.5; all expiry 2026-05-18; block on Tier 1.5 walk session)** = **44 Active rows total** · expiry 2026-05-17 (P1/P2/most P3) and 2026-05-18 (IMPL-013 + IMPL-034 + IMPL-039 + all 9 P4 rows) · all blocked on next Tier 1.5 walk session via `bootstrap_smoke.ini` headless backtest (IMPL-060 .ex5 now exists — runnable surface gap closed)
...
>
> **Next:** **Tier 1.5 Exploratory Walk** — close foreground MT5 → run `bash .agents/skills/mt5-headless-backtest/scripts/run_headless_backtest.sh simulation/headless-tests/bootstrap_smoke.ini /tmp/bootstrap_run.txt` → grep `[ev=init_ok]` + journal sample → produces walk artifact ≤14d that drains 44 Active deferred-AC rows. **AND** `/impl-review all` R09 (cross-slot + Orchestrator + entry .mq5 + ODR fix = significant new attack surface). **THEN** P4 tail: IMPL-061 (M baseline parser) → IMPL-062 (M Bucket A regression) → IMPL-063 (M Bucket B regression) → IMPL-064/065/066/067/068 + IMPL-017.
...
- ☐ **Tier 1.5 Exploratory Walk** (NOW UNBLOCKED): close foreground MT5 → `bash .agents/skills/mt5-headless-backtest/scripts/run_headless_backtest.sh simulation/headless-tests/bootstrap_smoke.ini /tmp/bootstrap_run.txt` → grep `[ev=init_ok]` + journal sample → produces walk artifact ≤14d that drains 44 deferred-AC rows
- ☐ Code Review Round R09 (`/impl-review all`) — recommended (cross-slot + Orchestrator + entry .mq5 + ODR fixes = significant new attack surface)
...
- **E-AC**:
  ...
  - [ ] G2 smoke: attach to EURUSD H4 → `[ev=init_ok]` log within 5 ticks `[log-assertion]` — **deferred** to Tier 1.5 walk via `simulation/headless-tests/bootstrap_smoke.ini` (foreground MT5 active during closure session = data-dir lock); registered in `deferred-ac-registry.md § Active` row IMPL-060 expires 2026-05-18; will drain together with 43 other rows on next Tier 1.5 walk session
- **Deps**: IMPL-059
- **Risk**: low
- **ADR**: ADR-012
- **Rules**: `.claude/rules/ea.md`
- **Closed**: 2026-05-04 (commit pending) — `MQL5/Experts/PhoenicisNex/PhoenicisNex.mq5` (NEW; 87 LOC). 3/3 S-AC `[x]`; 1/2 E-AC `[x]` (G1 PASS); 1 E-AC deferred (G2 live smoke → Tier 1.5 walk) → 1 new Active row in `deferred-ac-registry.md` expiry 2026-05-18. **Runnable surface gap closed** — `.ex5` produced 0err/0warn/3566 ms (Tester sandbox auto-pulls `Examples\ZigZag` indicator per `iCustom` usage in CrossSlotCoordinator IMPL-057). Evidence: `_session-handoff/IMPL-060-evidence-20260504.md`. P4 Phase Status snapshot 7/17 → 8/17. Mid-Phase Audit P4 counter = 3 (post-IMPL-057 GREEN audit). Plan Staleness Sentinel: 1 closure since R07 review (IMPL-060) — within threshold ✅.
