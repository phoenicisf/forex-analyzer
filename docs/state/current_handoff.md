...
**🟢 IMPL-FIX-013 (BT-002 impl-code cleanup) ✅ APPLIED 2026-05-18 — G1 PASS across all 3 entry points (PhoenicisNex.mq5 + PhoenicisNex_TickLatencyProbe.mq5 + spike/Spike_Orchestrator.mq5 = 0 errors / 0 warnings each).** Scope per `backtrack-log.md § BT-002 § Impacted phases → Impl Code` + TD `claim-review-10.md § Recommendation` operator-action #3: (1) DELETED `services/CircuitBreaker.mqh` + `spike/Spike_CircuitBreaker.mq5`; (2) stripped `core/Orchestrator.mqh` — `#include "../services/CircuitBreaker.mqh"`, `m_breaker` member field, ctor init list entry, Phase B step 10 `m_breaker.Init(m_logger)`, `PHOENICISNEX_WIRE(m_breaker, CCircuitBreaker)`, `_TeardownAll` step-10 delete, OnTick step 4 `m_breaker.CheckPingPong() → Halt("circuit_breaker_pingpong")` dispatch, entire `OnTradeTransaction(MqlTradeTransaction&, MqlTradeRequest&, MqlTradeResult&)` method (~100 LOC body containing ADR-013 DEAL_REASON filter + ADR-014 DEAL_ENTRY dedup + IsPhoenicisMagic gate + DEAL_TYPE filter + RecordOpen/RecordClose dispatch), `m_init_complete` lifecycle flag (was gate for trade-transaction producer), and D-2/D-8/D-9 banner comment sections (replaced with single BT-002 retired-banner note); (3) stripped `PhoenicisNex.mq5` + `PhoenicisNex_TickLatencyProbe.mq5` MT5-lifecycle `OnTradeTransaction` exports (g_orchestrator.OnTradeTransaction forwarder removed); (4) updated `spike/Spike_Orchestrator.mq5` IsPhoenicisMagic SelfTest banner + WireServices count comments (19 → 11 services + 4 helpers + 4 core peers). Halt triggers now reduce to indicator handle-invalid (OnTick step 5) + sustained journal failure (OnTick step 13b) per ADR-010 (amended 2026-05-17 per BT-002). `IsPhoenicisMagic` magic-membership gate in `domain/EnumTypes.mqh` PRESERVED as general foreign-EA filter for any future trade surface (no longer cited as CircuitBreaker producer guard). G2/G3 verification + IMPL-062 Bucket A 5-yr regression re-run pending operator session (next-step pivot — NFR-1.1 acceptance signal on rewrite-no-detector default build).
